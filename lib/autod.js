...
var REQUIRE_REG = /require\s*\(['"](@[0-9a-zA-Z\-\_][0-9a-zA-Z\.\-\_]*\/)?([0-9a-zA-Z\-\_][0-9a-zA-Z\.\-\_]*)(\/?([0-9a-zA-Z\.\-\_]*))*['"]\)/g;
...
var Autod = function (options) {
  ...
  this.registry = options.registry || 'http://registry.npm.taobao.org';
...
Autod.prototype.requestNpmAll = function (name, range, callback) {
  ...
  urllib.request(url, options, function (err, result) {
    ...
    if (!version) {
      var msg = util.fmt('can not found any match version of %s with range of %s', name, range);
...
Autod.prototype.parse = function (callback) {
  ...
  this.testRoots.forEach(function (testRoot) {
    devDeps = devDeps.concat(getDeps(this.findJsFile(testRoot)));
  }.bind(this));
  // add devdeps input
